...
class PERSON_DETECT(KPU_BASE):
    ...
    _NUM_CHANNELS = _NUM_ANCHORS * _NUM_OUTPUTS_PER_ANCHOR  # 18
    ...
    def __init__(self,
        ...
        @size: 模型输入尺寸，默认 640
        @kmodel_path: 模型路径，不传则使用自带的 person_detect_yolov5n.kmodel
        @nncase_version: nncase 版本
        """
        _RES_DIR = os.path.dirname(os.path.abspath(__file__))

        if kmodel_path is None:
            kmodel_path = os.path.join(_RES_DIR, "person_detect_yolov5n.kmodel")
    ...
    def run(self, img,
            ...
            nms_threshold: float = None) -> List[PERSON_DETECT_RESULT]:
        """同步检测图片中的人体"""
    ...
    def get_result(self) -> List[PERSON_DETECT_RESULT]:
        """获取最近一次检测的结果"""
    ...
    def ai2d_init(self, model_w: int, model_h: int,
        ...
        YOLO 模型是用居中 padding 训练的，grid_y 的权重与绝对位置绑定，
        顶部对齐会破坏这种绑定导致预测坐标偏移。
        """
        ...
        # 居中 padding（匹配原始 personDet.py 的 get_padding_param）
        pad_left = dw // 2
    ...
    # ============================================================
    # 后处理（YOLOv5n anchor-based 解码 + NMS）
    # ============================================================

    def post_process(self,
                     reliability_threshold: float,
                     nms_threshold: float) -> List[PERSON_DETECT_RESULT]:
        """
        YOLOv5n 后处理：
        ...
        for scale_idx, stride in enumerate(self._STRIDES):
            ...
            for a in range(self._NUM_ANCHORS):
                ...
                cls_raw = output[a, 5].astype(np.float32)

                # ---- YOLOv5 解码 ----
                # nncase 编译时通常已融合 Sigmoid，模型输出已在 [0,1] 范围
                # 直接使用原始值，不再重复应用 sigmoid
                # bx = (tx*2 - 0.5 + grid_x) * stride
                # by = (ty*2 - 0.5 + grid_y) * stride
                # bw = anchor_w * (tw*2)²
                # bh = anchor_h * (th*2)²
                ...
                # ---- 置信度（obj, cls 已被 nncase sigmoid） ----
                scores = obj_raw * cls_raw
                ...
                # 3. 坐标映射：模型空间 → 原图空间
                #
                # 与 aicube.anchorbasedet_post_process 一致：
                # 根据 model_input_size 和 rgb888p_size 计算居中 padding，
                # 然后 (坐标 - pad) / ratio 还原到原图尺寸
                # ---------------------------------------------------
...
def _reshape_output(raw: np.ndarray, stride: int,
    ...
    nncase 编译的 YOLO 模型常见输出格式:
      - (1, C, N)    扁平化 NCHW（最常见，参考 YOLO11_DET）
      - (1, C, H, W) 空间 NCHW
      - (1, H, W, C) 空间 NHWC
